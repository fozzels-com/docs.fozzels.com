---
id: '103000392433'
title: 2.3.1. Aplicativo Fozzels Shopify — Começando
sidebar_position: 3
slug: /integration-connectivity/fozzels-shopify-app-getting-started
description: >-
  Obrigado por instalar o Aplicativo Fozzels Shopify! Estamos entusiasmados em
  ajudá-lo a automatizar seu conteúdo de produto e economizar horas de trabalho manual.
---

#

Obrigado por instalar o Aplicativo Fozzels Shopify! Estamos entusiasmados em ajudá-lo a automatizar seu conteúdo de produto e economizar horas de trabalho manual. Vamos configurar você em apenas alguns minutos.

##
O que é o Aplicativo Fozzels Shopify?

O Aplicativo Fozzels Shopify é a integração oficial entre sua loja Shopify e a plataforma de geração de conteúdo de IA Fozzels. Uma vez conectado, o Fozzels pode extrair seus produtos diretamente do Shopify, gerar descrições, títulos de SEO e outros conteúdos únicos alimentados por IA para cada produto e sincronizar automaticamente os resultados de volta para sua loja.

**Com o Aplicativo Fozzels Shopify você pode:**

-   Conecte sua loja Shopify ao Fozzels em minutos — nenhuma chave de API necessária
-   Extraia todos os seus produtos e seus atributos para o Fozzels automaticamente
-   Gere conteúdo único para cada produto usando IA
-   Sincronize conteúdo gerado de volta para Shopify automaticamente ou após revisão manual
-   Gere imagens e vídeos de produtos de IA e publique-os na galeria de mídia de sua loja
-   Gerencie vários mercados Shopify e idiomas em um único lugar

##
Requisitos

Antes de começar, certifique-se de ter:

-   Uma loja Shopify (qualquer plano)
-   Uma conta Fozzels — [Inscreva-se aqui](https://app.fozzels.com/signup)
-   O aplicativo Fozzels instalado pela Shopify App Store

##
Etapa 1 — Instale o Aplicativo

1.  Vá para a [página do aplicativo Fozzels](https://apps.shopify.com/fozzels) na Shopify App Store
2.  Clique em **Add app**
3.  Revise as permissões solicitadas e clique em **Install**
4.  Você será redirecionado para a tela de conexão Fozzels dentro do seu Shopify Admin

##
Etapa 2 — Conecte Sua Conta Fozzels

Assim que o aplicativo for instalado, você verá a tela **Connect to Fozzels** em seu Shopify Admin.

1.  Clique em **"Login to Fozzels"**
2.  Digite o email e senha da sua conta Fozzels
3.  Clique em **Login**

> Ainda não tem uma conta Fozzels? Clique em **"Create Account"** para se inscrever diretamente do Shopify.

Após fazer login, o Fozzels detectará sua loja e abrirá o diálogo **Select Shopify Integration**.

##
Etapa 3 — Crie uma Nova Integração

Se essa é sua primeira vez conectando, clique em **"+ Create new integration"**. Você será levado ao painel Fozzels para completar a configuração.

1.  Na tela **Choose your integration** — selecione **Shopify**
2.  Preencha o formulário de integração:
    -   **Name** — dê um nome reconhecível para sua integração (por exemplo, "My Shopify Store")
    -   **URL** — digite sua URL de loja (por exemplo, `https://yourstore.myshopify.com`)
    -   **Connection Method** — selecione **Fozzels Shopify App (OAuth)** _(recomendado)_
    -   **App Host Name** — digite seu nome de host `.myshopify.com` (por exemplo, `yourstore.myshopify.com`)
3.  Opcionalmente ative **Shopify Markets** se você executa uma loja multilíngue ou multi-região
4.  Clique em **Save**

> **Por que OAuth?** O método OAuth é a forma mais fácil e segura de conectar. Ele não requer chaves de API manuais ou tokens — o token de acesso é fornecido automaticamente quando você se conecta via aplicativo Shopify.

##
Etapa 4 — Autorize a Conexão

Após salvar a integração, volte ao seu Shopify Admin:

1.  Abra o **aplicativo Fozzels** no Shopify Admin
2.  Clique em **"Login to Fozzels"** e faça login
3.  No diálogo **Select Shopify Integration** — selecione sua integração recém-criada
4.  Clique em **Connect**

Você verá a guia **Integrations** atualizar com um status **Active**. No painel Fozzels, a integração mostrará **Authorized ✅** e **REST API Connected ✅**.

##
Etapa 5 — Extraia Seus Produtos

1.  Vá para `app.fozzels.com` → **Integrations** → abra sua integração Shopify
2.  Navegue até a guia **Websites & Stores**
3.  Clique em **"Pull Websites and Stores"** para carregar todos os mercados e idiomas disponíveis
4.  Ative o toggle **Status** para as lojas com as quais você deseja trabalhar
5.  Clique em **"Pull Products"** para cada loja
6.  Aguarde até que a barra de progresso atinja **100%**

Seus produtos estão agora disponíveis no **Catalog** do Fozzels, completos com todos os seus atributos — prontos para geração de conteúdo de IA.

##
Qual é o Próximo Passo?

Assim que seus produtos forem extraídos, você pode:

-   Navegue e filtre seus produtos no **Catalog**
-   Crie um **Content Flow** para gerar descrições de IA, títulos de SEO e muito mais
-   Configure um **Image Flow** para gerar fotos profissionais de produtos usando IA
-   Use **Video Flow** para criar vídeos de produtos automaticamente

→ [Aprenda como criar seu primeiro Content Flow](/content-creation-flows/creating-a-new-content-flow-and-initial-settings)
→ [Aprenda sobre Image Flows](/content-creation-flows/user-guide-automated-image-flow/)

_Precisa de ajuda? Entre em contato conosco em [support@fozzels.com](mailto:support@fozzels.com)_
