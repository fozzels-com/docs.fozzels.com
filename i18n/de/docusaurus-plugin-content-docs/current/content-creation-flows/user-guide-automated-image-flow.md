...
description: >-
  Image Flow ist ein professionelles Tool, das für die Massengenerierung und Synchronisierung von Produktbildern mit KI entwickelt wurde. Durch die einmalige Konfiguration eines Flows etablieren Sie ein autonomes System, das tausende Produkte verarbeitet.
---
...
> **Wichtig:** Wir empfehlen dringend, den Flow **nicht zu aktivieren** (den Schalter "Aktiver Flow" **AUS** lassen), bis Sie alle Konfigurationen vollständig abgeschlossen und Ihre Einstellungen getestet haben.

## 1\. Erstellung eines neuen Image Flow (Registerkarte 1)

Diese Registerkarte handhabt die grundlegende Identität und Verbindung Ihrer Automatisierung. Es gibt zwei Hauptmethoden, um einen neuen Flow zu initiieren:

-   **Option A: Über das Menü Image Flows** - Navigieren Sie zum Abschnitt **Image Flows** in der oberen Navigationsleiste und klicken Sie auf die Schaltfläche **Neuer Image Flow**. Wählen Sie Ihre Integration, Website und Store nacheinander aus den Dropdown-Menüs.
    ![](/img/kb/content-creation-flows/user-guide-automated-image-flow/aGysMRzpl5ijAiHFUH5TFnasSdIEP1py9w.png)

-   **Option B: Aus dem Produktkatalog** - Gehen Sie zum Abschnitt **Katalog → Produkte**, filtern Sie die Produkte, die Sie verarbeiten möchten, wählen Sie diese aus und klicken Sie auf **Aktionen → Image Flow erstellen**. Diese Methode ist schneller, da sie Ihren Store und den Produktauswahlkontext automatisch vorausfüllt.
    ![](/img/kb/content-creation-flows/user-guide-automated-image-flow/WVP7KcJNwsPTLqIzLSQQwGsAoBCdxAdqLg.png)

**Wesentliche Schritte:**

1.  **Geben Sie Ihrem Flow einen Namen:** Geben Sie Ihrem Flow einen klaren, beschreibenden Namen (z. B. "Sommerkleid 2026 - Gemini Pro").

2.  **Speichern Sie Ihren Fortschritt:** Alle Änderungen am Flow-Namen oder der Store-Auswahl müssen durch Klicken auf die Schaltfläche **Absenden** am Ende der Seite bestätigt werden.
...
Wenn Sie das Modell **GPT Image 1** wählen, haben Sie Zugriff auf die folgenden Parameter:

-   **Qualität**: Wählen Sie Ihre bevorzugte Generierungsqualität aus dem Dropdown-Menü (**Auto, Hoch, Mittel oder Niedrig**).

-   **Bildgröße**: Wählen Sie das gewünschte Format aus dem Dropdown-Menü (**Auto, Quadrat, Landschaft oder Hochformat**). Hinweis: Ein interaktives Gitter für GPT kommt bald.

-   **Bildanzahl**: Sie können **zwischen 1 und 4 Variationen** für jedes Produkt pro Ausführung generieren, die mehrere Optionen zur manuellen Überprüfung bieten.

-   **Technische Limits**: Die maximale Eingabedateigröße für GPT ist **50 MB**.
...
### **Abschnitt: Vorlagen auswählen**

Vorlagen sind visuelle Referenzbilder, die als Anweisungen für die KI fungieren. Sie legen den Gesamtstil, die Beleuchtung und den Kontext für **jedes** Produkt fest, das innerhalb dieses Flows verarbeitet wird.

> **Die goldene Regel: Universalität** Da ein einzelner Satz von Vorlagen für eine gesamte Gruppe von Produkten verwendet wird (z. B. hunderte Kleider oder alle Schuhe), müssen Ihre Auswahl **UNIVERSELL** sein.
>
> -   _Beispiel:_ Falls Sie eine **Produkt**-Vorlage hinzufügen, die eine bestimmte blaue SKU zeigt, könnte die KI versuchen, blaue Details zu jedem anderen Element im Flow hinzuzufügen. Wählen Sie nur Referenzen, die für die gesamte Produktkategorie, die Sie verarbeiten, angemessen sind.
>
